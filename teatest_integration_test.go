...
// TestTeatestSpaceToggleMark verifies that Space toggles the mark on
// the current entry.
func TestTeatestSpaceToggleMark(t *testing.T) {
	tm := newTeatestModel(t, []string{"a", "b", "c"})
	...
	// Press Space to mark the first item.
	tm.Send(tea.KeyMsg{Type: tea.KeyRunes, Runes: []rune{' '}})
	...
	if !m.marked[0] {
		t.Error("expected index 0 to be marked after Space")
	}
}

// TestTeatestSpaceToggleMarkTwice verifies that pressing Space twice
// unmarks the entry.
func TestTeatestSpaceToggleMarkTwice(t *testing.T) {
	tm := newTeatestModel(t, []string{"a", "b"})
	...
	if m.marked[0] {
		t.Error("expected index 0 to be unmarked after double Space")
	}
}

// TestTeatestVimNormalSpaceToggleMark verifies Space toggles marks in
// vim normal mode too.
func TestTeatestVimNormalSpaceToggleMark(t *testing.T) {
	withIsolatedEnv(t)
	m := newModel(false, false, true)
	m.src = srcFiles
	m.sessionPaths = map[string]string{}
	...
	tm := teatest.NewTestModel(t, m, teatest.WithInitialTermSize(100, 30))
	time.Sleep(200 * time.Millisecond)
	tm.Send(itemsMsg{src: srcDefault, items: []string{"x", "y"}, info: map[string]sessionInfo{}})
	time.Sleep(100 * time.Millisecond)
	...
	if !m2.marked[0] {
		t.Error("expected index 0 to be marked after Space in normal mode")
	}
}
